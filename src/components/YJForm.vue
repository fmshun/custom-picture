    ...
    >
      <el-carousel-item v-for="(item, k) in ruleForm.channels" :key="k">
        ...
        <div
          ...
          class="img-show"
          :style="{
            height: handleHeight(item),
            'border-radius': needRadius.includes(item) ? '18px' : 0,
          }"
        >
          ...
          <div
            class="which-round"
            v-else-if="item == '华为 1280x720首页'"
            :style="countStyle(item, 4)"
          >
            意甲{{ ruleForm.title }}
          </div>
          <div
            v-else-if="!onlyImgs.includes(item)"
            class="which-round"
            :style="countStyle(item, 4)"
          >
            <div>意大利足球</div>
            <div class="round-num">甲级联赛{{ ruleForm.title }}</div>
          </div>
          <div class="middle-team" v-if="!onlyImgs.includes(item)">
          ...
          <div
            ...
            v-if="!onlyImgs.includes(item)"
          >
            <span v-if="compeType == '1' && !noStartTime.includes(item)">
          ...
          <img
            class="bg-r-img"
            v-if="!noRightChannels.includes(item)"
...
const onlyImgs = ref(["小米 1056x594", "海信 411x232", "华为 1280x720赛果"]); // 纯素图
const noRightChannels = ref(["海信 560x315"]); // 无右边背景图的渠道
const noStartTime = ref(["小米 1632x720"]); // 无赛事预告时间的
const needRadius = ref(["当贝 824x303"]); // 模板有圆角的

const checkAll = ref(false);
const indeterminate = ref(false);
const channelOpions = ref(constant.YJ_CHANNEL);
const teams = ref(constant.YJ_LOGO);
...
const getImg = (item: string) => {
  return new URL(`../assets/img/YJ_CHANNEL/${item}.png`, import.meta.url).href;
};
