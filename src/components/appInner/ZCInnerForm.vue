    ...
    >
      <el-carousel-item v-for="(item, k) in ruleForm.channels" :key="k">
        ...
        >
          ...
          <div class="which-round" :style="countStyle(item, 4)">
            {{ ruleForm.title }}
          </div>
...
const channelOpions = ref(constant.ZC_INNER);
...
const getImg = (item: string) => {
  return new URL(`../../assets/img/ZC_INNER/${item}.png`, import.meta.url).href;
};
