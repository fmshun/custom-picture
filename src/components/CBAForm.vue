  ...
  <div class="form-box">
    ...
    >
      <el-form-item label="渠道" prop="channels">
        <el-col :span="6">
          <el-select v-model="compClassify" placeholder="请选择">
            <el-option label="常规赛" value="0" />
            <el-option label="季后赛" value="1" />
          </el-select>
        </el-col>
        <el-col :span="18">
          <el-select
...
// 监听赛事分类(如常规赛、季后赛)
watch(compClassify, (v: string) => {
  ruleForm.channels = [];
  curKey.value = 0;
  channelOpions.value = constant.CBA_CHANNEL[v];
});
// 渠道更改监听
watch(
  () => ruleForm.channels,
...
const getImg = (item: string) => {
  if (item == "小米 1632x720" && compeType.value == "2") {
    return new URL(`../assets/img/CBA_CHANNEL/${item}_2.png`, import.meta.url)
      .href;
  }
  return new URL(`../assets/img/CBA_CHANNEL/${item}.png`, import.meta.url).href;
};
